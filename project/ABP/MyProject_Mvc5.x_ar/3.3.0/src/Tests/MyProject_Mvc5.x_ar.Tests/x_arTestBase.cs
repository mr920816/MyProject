...
namespace MyProject_Mvc5.x_ar.Tests
{
    public abstract class x_arTestBase : AbpIntegratedTestBase<x_arTestModule>
    {
        ...
        protected void UsingDbContext(Action<x_arDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<x_arDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<x_arDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<x_arDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<x_arDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<x_arDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<x_arDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<x_arDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<x_arDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<x_arDbContext, Task<T>> func)
        {
            T result;
